...
const CONFIG_PATH_CONFIRMED_KEY = "configPathConfirmed";
...
function syncConfigPathUI() {
  ...
  const firstRunNotice = $("#firstRunConfigNotice");
  if (firstRunNotice) {
    const confirmed = localStorage.getItem(CONFIG_PATH_CONFIRMED_KEY) === "1";
    firstRunNotice.hidden = confirmed || Boolean(state.lists?.config_exists);
  }
}
...
async function continueStartupAfterConfig() {
...
async function applyConfigPathOverride() {
  ...
  await ensureSelectedConfigFile();
  syncConfigPathUI();
  await loadConfigLists();
  setStatus(t("statusSaved"));
  await continueStartupAfterConfig();
}
...
async function ensureSelectedConfigFile() {
  if (!tauriInvoke) return;
  const lists = await invoke("ensure_config_file", { path: state.configPath || null });
  if (!lists) return;
  state.lists = lists;
  state.configPath = lists.config_path;
  localStorage.setItem(CONFIG_PATH_CONFIRMED_KEY, "1");
}

function shouldAskForConfigPath(configOverride) {
  if (localStorage.getItem(CONFIG_PATH_CONFIRMED_KEY) === "1") return false;
  if (configOverride) return false;
  return !state.lists?.config_exists;
}
...
async function boot() {
  ...
  try {
    ...
    syncConfigPathUI();
    await loadConfigLists();
    if (shouldAskForConfigPath(configOverride)) {
      setTab("settings");
      syncConfigPathUI();
      setStatus(t("firstRunConfigStatus"));
      return;
    }
    await continueStartupAfterConfig();
