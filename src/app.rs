...
const DEFAULT_UPDATE_REPO: &str = "fantasylidong/anne-server-browser";
...
fn default_gui_config_path() -> PathBuf {
    executable_sidecar_dir()
        .unwrap_or_else(|| env::current_dir().unwrap_or_else(|_| PathBuf::from(".")))
        .join(DEFAULT_CONFIG_FILE)
}

fn executable_sidecar_dir() -> Option<PathBuf> {
            ...
            if let Some(contents_dir) = exe_dir.parent() {
                if contents_dir.file_name().and_then(|name| name.to_str()) == Some("Contents") {
                    if let Some(app_bundle) = contents_dir.parent() {
                        if app_bundle.extension().and_then(|name| name.to_str()) == Some("app") {
...
#[command(about = "Standalone Left 4 Dead 2 server browser with groups, subscriptions, RCON, and AnneWeb integration.")]
struct Cli {
    #[arg(long)]
...
pub fn tauri_ensure_config_file(config_path: Option<String>) -> Result<TauriConfigLists, String> {
    let path = tauri_path(config_path);
    if !path.exists() {
        save_config(&path, &BrowserConfig::default())?;
    }
...
fn fetch_latest_update_tag_from_atom(
    ...
    let repo = update_repo();
    let url = format!("https://github.com/{repo}/{feed_path}");
    let text = client
        .get(&url)
...
fn fetch_release_assets(client: &Client, tag_name: &str) -> Result<Vec<ReleaseAsset>, String> {
    let repo = update_repo();
    let url = format!("https://github.com/{repo}/releases/expanded_assets/{tag_name}");
    let text = client
        .get(&url)
        .header("User-Agent", USER_AGENT)
        .send()
...
fn update_repo() -> &'static str {
    option_env!("ANNE_SERVER_BROWSER_UPDATE_REPO").unwrap_or(DEFAULT_UPDATE_REPO)
}
...
mod tests {
    ...
    fn compares_release_versions() {
        ...
        assert!(!is_update_release_tag(
            "anne-web-stable-release-2026-05-09"
        ));
    ...
    fn extracts_latest_release_tag_from_atom_feed() {
        let feed = r#"
            <feed>
              <entry>
                <link href="https://github.com/fantasylidong/Anne刷服器/releases/tag/Anne%E5%88%B7%E6%9C%8D%E5%99%A8-v0.8.0"/>
              </entry>
              <entry>
                <link href="https://github.com/fantasylidong/Anne刷服器/releases/tag/Anne%E5%88%B7%E6%9C%8D%E5%99%A8-v0.9.1"/>
              </entry>
              <entry>
                <link href="https://github.com/fantasylidong/Anne刷服器/releases/tag/anne-web-stable-release-2026-05-09"/>
              </entry>
            </feed>
        "#;
    ...
    #[test]
    fn default_gui_config_is_sidecar_toml() {
        assert_eq!(
            default_gui_config_path().file_name().and_then(|name| name.to_str()),
